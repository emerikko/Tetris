...
class Drawing:
    ...
    def draw_board(self, board):
        ...
        for y, row in enumerate(board):
            for x, cell in enumerate(row):
                ...
                # pygame.draw.rect(self.screen, (100, 100, 100), (x * block_size, y * block_size, block_size, block_size), 1)
    ...
    def draw_board_borders(self, board):
        ...
        for y in range(len(board)):
            pygame.draw.line(self.screen, (100, 100, 100), (0, y * block_size), (len(board[0]) * block_size, y * block_size), 2)

        # Вертикальные линии
        for x in range(len(board[0])):
            pygame.draw.line(self.screen, (100, 100, 100), (x * block_size, 0), (x * block_size, len(board) * block_size), 2)

        # Границы поля
        pygame.draw.rect(self.screen, (200, 200, 200), (0, 0, len(board[0]) * block_size, len(board) * block_size + 1), 2)
    ...
    def draw_shape(self, shape, offset):
        ...
        for y, row in enumerate(shape):
            for x, cell in enumerate(row):
                if cell:
                    pygame.draw.rect(self.screen, (0, 255, 0), (x * block_size + offset[0] * block_size, y * block_size + offset[1] * block_size, block_size, block_size), 0)
    ...
    def draw_ui(self, score, level, next_shape):
        ...
        self.screen.blit(next_text, (screen_width - next_text.get_width() - 10, 70))
